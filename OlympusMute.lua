...
local MATCH = "olympus"   -- case-insensitive substring of the guild name
...
local function IsOlympus(guild)
    return type(guild) == "string" and guild:lower():find(MATCH, 1, true) ~= nil
end
...
CreatePanel = function()
    ...
    desc:SetText("Hides chat from players whose guild name contains \"Olympus\". "
    ...
    local whoBtn = MakeButton("Scan /who g-Olympus", 200, WhoScan, declineGroup, nil, -10)
    function panel:UpdateScanButton()
        local total = (scanQueries and #scanQueries) or #BuildScanQueries()
        whoBtn:SetText(("Scan /who g-Olympus (%d/%d)"):format(scanStep % total + 1, total))
    end
    local clearBtn = MakeButton("Clear list", 110, function() StaticPopup_Show("OLYMPUSMUTE_CLEAR") end, whoBtn, 8, 0)
...
SlashCmdList.OLYMPUSMUTE = function(input)
    ...
    elseif cmd == "" or cmd == "config" or cmd == "options" then
        ...
        Print("/omute (opens options) | on | off | guildinvites on|off | groupinvites on|off | list | scan | check Name | add Name | remove Name | clear")
    end
end
